...
router.get('/allBought', async (req, res) => {
    try {
        const [rows] = await db.query('SELECT lotto_number FROM LottoAll WHERE lotto_status = "ถูกซื้อไปแล้ว"');
        res.json(rows);
    } catch (error) {
        res.status(500).send('Error fetching lotto');
    }
...
router.post('/myLotto', async (req, res) => {
    ...
    try {
        const checkQuery = 'SELECT LottoAll.lotto_id ,LottoAll.lotto_number,LottoBought.user_id FROM LottoBought LEFT JOIN LottoAll WHERE LottoBought.user_id = ?';
